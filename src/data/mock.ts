...
export const LIVE_INCIDENTS: Incident[] = [
  { id: 'INC-01', type: 'roadblock', severity: 'critical', node_id: 'stadium_jn', affected_links: ['road-bus-stand-stadium', 'road-stadium-mananchira', 'road-stadium-poonthanam'], status: 'active', reported_at: Date.now() - 16 * 60 * 1000, title: 'Event spillover at Stadium corridor', note: 'Queue propagation detected on event-sensitive links.' },
  { id: 'INC-02', type: 'breakdown', severity: 'high', node_id: 'palayam_jn', affected_links: ['road-poonthanam-palayam', 'road-mananchira-palayam'], status: 'responding', reported_at: Date.now() - 28 * 60 * 1000, title: 'Bus dwell time rise at Palayam', note: 'Loading activity is extending cycle times.' },
  { id: 'INC-03', type: 'vip', severity: 'medium', node_id: 'bus_stand_jn', affected_links: ['road-mavoor-bus-stand', 'road-bus-stand-arayidathupalam', 'road-bus-stand-stadium'], status: 'active', reported_at: Date.now() - 9 * 60 * 1000, title: 'VIP convoy approaching transit hub', note: 'Priority route is reserved through Arayidathupalam.' },
];

export const DEFAULT_EVENTS: EventPlan[] = [
  { id: 'EV-01', name: 'EMS Stadium football match', type: 'sports', venue_node: 'stadium_jn', datetime: '2026-05-21T18:30:00+05:30', crowd_size: 32000, duration: 150, active: true, dispersal: 30 },
  { id: 'EV-02', name: 'Palayam cultural rally', type: 'rally', venue_node: 'palayam_jn', datetime: '2026-05-21T14:00:00+05:30', crowd_size: 12000, duration: 80, active: false, dispersal: 0 },
];
...
export const getSnapshot = (tick = 0, loadFactor = 0) => {
  const linkStates: LinkState[] = LINKS.map((link) => buildLinkState(link, nodeMap, tick, loadFactor));
  const nodeStates: NodeState[] = NODES.map((node) => buildNodeState(node, tick));
  ...
  const overallCongestion = round1(linkStates.reduce((sum, link) => sum + link.congestion_index, 0) / linkStates.length);
